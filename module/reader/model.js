...
var db  = require('../db.js')
	...
	, SQL = {
		...
		, bookmarkAdd: 'insert into reader_bookmark(url,title,source,tags,datetime,status,create_user_id) select :url,:title,:source,:tags,now(),:status,:userId from dual where not exists (select * from reader_bookmark where url like :url)'
		, bookmarkUpdateRead: 'update reader_bookmark set status=2,title=:title,tags=:tags,score=score+:score,num_reader=num_reader+1 where Id=:id'
